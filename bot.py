    ...
    try:
        ...
        if result:
            ...
            if isinstance(value, dict):
                ...
                value.setdefault(
                    "stats", {"images_generated": 0, "questions_answered": 0}
                )
                value["stats"].setdefault("images_generated", 0)
                value["stats"].setdefault("questions_answered", 0)
                return value
...
def ensure_user(user):
    ...
    profile.setdefault("language", "English")
    profile.setdefault("joined_date", time.strftime("%Y-%m-%d"))
    profile.setdefault("daily_facts_opt_in", True)
    profile.setdefault("trivia_seen", [])
...
async def generate_image(update, context, prompt):
    ...
    try:
        ...
        keyboard = [
            ...
            [
                InlineKeyboardButton("⭐ Save to Favorites", callback_data="favorite"),
            ],
        ]
...
async def help_command(update, context):
    await update.effective_message.reply_text(
        ...
        "/image <prompt>\n"
        "/design <idea>\n"
        "/ask <question> (or just talk, or send a voice note)\n"
        "/styles — pick a style preset\n"
        "/history — redo a past prompt\n"
        "/favorites — your saved images\n"
        "/language — set the language I reply in\n\n"
        "💎 PREMIUM\n"
        "/premium\n"
        "/confirm\n"
        ...
        "/togglefacts — opt in/out of daily facts\n\n"
        "👤 ACCOUNT\n"
        "/myid\n\n"
        "🛠 ADMIN\n"
        "/customers\n"
...
async def favorites_command(update, context):
    ...
    if not favorites:
        await update.effective_message.reply_text(
            ...
            "Tap \"⭐ Save to Favorites\" under any image you like to add it here."
        )
        return

    recent = favorites[-5:]
    context.user_data["favorites_recent"] = recent

    keyboard = [
        [InlineKeyboardButton(f"🔄 {p[:30]}", callback_data=f"redofav:{i}")]
        for i, p in enumerate(recent)
    ]

    await update.effective_message.reply_text(
        "⭐ YOUR FAVORITES\n\n"
        "Tap one to generate it again.",
        reply_markup=InlineKeyboardMarkup(keyboard),
    )
...
async def language_command(update, context):
    ...
    await update.effective_message.reply_text(
        ...
        f"Current: {profile['language']}\n\n"
        "I'll reply to /ask and normal messages in this language.",
        reply_markup=InlineKeyboardMarkup(keyboard),
    )
        ...
        messages = [
            {
                "role": "system",
                "content": (
                    f"Always reply in {profile['language']}, no matter what "
...
async def togglefacts_command(update, context):
    profile = ensure_user(update.effective_user)
    profile["daily_facts_opt_in"] = not profile["daily_facts_opt_in"]
    await save()

    if profile["daily_facts_opt_in"]:
        await update.effective_message.reply_text(
            "✅ You'll get a fun fact every day."
        )
    else:
        await update.effective_message.reply_text(
            "🚫 Daily facts turned off. Use /togglefacts again to turn them "
            "back on."
        )
...
TRIVIA_QUESTIONS = [
    ("What is the largest ocean on Earth?", ["Atlantic", "Pacific", "Indian"], 1),
    ("Which country has the largest population?", ["USA", "India", "Nigeria"], 1),
    ("What gas do plants absorb from the air?", ["Oxygen", "Carbon dioxide", "Nitrogen"], 1),
    ("How many continents are there?", ["5", "6", "7"], 2),
    ("What is the fastest land animal?", ["Lion", "Cheetah", "Horse"], 1),
    ("Which organ pumps blood around the body?", ["Liver", "Heart", "Kidney"], 1),
    ("What is the currency of Nigeria?", ["Cedi", "Naira", "Rand"], 1),
    ("Which planet is closest to the sun?", ["Venus", "Mercury", "Earth"], 1),
    ("How many players are on a football (soccer) team?", ["9", "10", "11"], 2),
    ("What is the tallest mountain in the world?", ["K2", "Everest", "Kilimanjaro"], 1),
    ("Which is the largest mammal on Earth?", ["Elephant", "Blue whale", "Giraffe"], 1),
    ("What language has the most native speakers worldwide?", ["English", "Mandarin Chinese", "Spanish"], 1),
    ("What do bees produce?", ["Milk", "Honey", "Silk"], 1),
    ("Which shape has three sides?", ["Square", "Triangle", "Pentagon"], 1),
    ("What is H2O more commonly known as?", ["Salt", "Water", "Oxygen"], 1),
]
...
async def trivia_command(update, context):
    ...
    seen = profile.setdefault("trivia_seen", [])
    remaining = [i for i in range(len(TRIVIA_QUESTIONS)) if i not in seen]

    # Once they've seen every question, start a fresh round instead of
    # locking them out.
    if not remaining:
        seen.clear()
        remaining = list(range(len(TRIVIA_QUESTIONS)))

    idx = random.choice(remaining)
    question, options, correct = TRIVIA_QUESTIONS[idx]
...
async def customers_command(update, context):
    ...
    premium_count = sum(1 for p in users.values() if p.get("premium"))
    new_today = sum(1 for p in users.values() if p.get("joined_date") == today)
    total_points = sum(p.get("points", 0) for p in users.values())
    active_groups = sum(
        1 for g in data["groups"].values() if g.get("premium")
    )
    ...
    await update.effective_message.reply_text(
        ...
        f"🏢 Active group plans: {active_groups}\n\n"
        f"🖼️ Images generated (all-time): {data['stats']['images_generated']}\n"
        f"💬 Questions answered (all-time): {data['stats']['questions_answered']}\n\n"
...
async def daily_fact_broadcast(context):
    ...
    for user_id, profile in list(data["users"].items()):
        if not profile.get("daily_facts_opt_in", True):
            continue
        ...
        try:
            ...
            failed += 1
            logger.warning("Daily fact failed for %s: %s", user_id, error)

    logger.info("Daily fact broadcast: sent=%s failed=%s", sent, failed)
...
FACTS = [
    ...
    "The shortest war in history lasted about 38 minutes. ⏱️",
...
FACT_BROADCAST_TIME_UTC = dt_time(hour=8, minute=0)
...
async def button(update, context):
    ...
    if query.data == "image_help":
        ...
        if not prompt:
            await query.message.reply_text(
                "Nothing to favorite yet — generate an image first."
            )
            return

        favorites = profile["favorites"]
        if prompt in favorites:
            await query.message.reply_text("⭐ Already in your favorites.")
            return

        favorites.append(prompt)
        del favorites[:-20]  # keep the most recent 20
        await save()

        await query.message.reply_text(
            "⭐ Saved to your favorites! Use /favorites to view them."
        )

    elif query.data.startswith("redofav:"):
        try:
            idx = int(query.data.split(":", 1)[1])
        except ValueError:
            return

        recent = context.user_data.get("favorites_recent", [])
        if idx >= len(recent):
            await query.message.reply_text("That favorite is no longer available.")
            return
        ...
        await query.message.reply_text(
            f"✅ Language set to {lang}. Try asking me something!"
        )
    ...
    elif query.data.startswith("trivia:"):
        profile = ensure_user(query.from_user)
        ...
        _, options, correct = TRIVIA_QUESTIONS[idx]
        ...
        if chosen == correct:
            ...
            await query.message.reply_text(
                ...
                f"+100 points\n"
                f"Total: {profile['points']:,}\n\n"
                f"Send /trivia for another question."
            )
        else:
            await save()
            await query.message.reply_text(
                f"❌ Not quite.\n\n"
                f"The correct answer was: {options[correct]}\n\n"
                f"Send /trivia for another question."
            )
...
async def handle_voice(update, context):
    ...
    listening = await message.reply_text("🎙️ Listening...")
    ...
    try:
        ...
        try:
            await listening.edit_text(
                "❌ I couldn't process that voice note. Please try again "
...
def main():
    ...
    app.add_handler(CommandHandler("togglefacts", togglefacts_command))
    ...
    if app.job_queue is not None:
        ...
        app.job_queue.run_daily(
            daily_fact_broadcast,
            time=FACT_BROADCAST_TIME_UTC,
        )
    else:
        logger.warning(
            "JobQueue not available — monthly referral leaderboard reset "
            "and daily facts will not run. Install with: "
